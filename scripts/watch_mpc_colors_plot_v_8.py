#!/usr/bin/env python3
"""
watch_mpc_colors_plot_v8.py
3I/ATLAS (C/2019 Y4) — Full Reproducibility Chain:
Color Evolution, Brightness Timeline, Solar Baselines,
Filter Activity, Δ Analysis, and OpenTimestamps Manifest.
...
def analyze_color_differences(pairs, perihelion, source_file):
    """Compute pre/post-perihelion color changes and export."""
    stats = []
    ...
    stats_path = Path("I3_Color_Statistics_v8.txt")
...
# Plot
# ------------------------------------------------------------
def plot_colors(pairs, df, stats):
    ...
    ax.set_title("3I/ATLAS — Color Evolution (v8)")
    ax.set_xlabel("Date (UTC, 2025)")
    ax.set_ylabel("Color Index (mag)")
    ax.grid(alpha=0.3)
    ax.legend()
    plt.tight_layout()
    plt.savefig("I3_Color_Trend_v8.png", dpi=300)
    plt.close(fig)
    print("✅ Saved: I3_Color_Trend_v8.png")

# ------------------------------------------------------------
# Proof Manifest
# ------------------------------------------------------------
def write_proof_manifest(source_file, sha_in, stats_file, pairs_file, plot_file):
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    manifest = Path("I3_Color_Proof_v8.txt")
    with open(manifest, "w") as f:
        ...
        f.write("\nTo verify integrity:\n")
        f.write("  ots stamp I3_Color_Proof_v8.txt\n")
        f.write("  ots verify I3_Color_Proof_v8.txt.ots\n")
    print("🔏 Proof manifest created: I3_Color_Proof_v8.txt")
...
if __name__ == "__main__":
    ...
    pairs_path = "I3_Color_Alerts.csv"
    pairs.to_csv(pairs_path, index=False)
    print(f"✅ Saved {len(pairs)} color pairs → {pairs_path}")

    stats, stats_path, sha_in = analyze_color_differences(pairs, PERIHELION, src)
    plot_colors(pairs, df, stats)
    write_proof_manifest(src, sha_in, stats_path, pairs_path, "I3_Color_Trend_v8.png")
