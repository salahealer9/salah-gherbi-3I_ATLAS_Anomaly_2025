#!/usr/bin/env python3
"""
watch_mpc_colors_plot_v7_6.py
3I/ATLAS (C/2019 Y4) — MPC Color Evolution + Dual-Axis Brightness Timeline
Includes perihelion highlight, solar references, and filter-activity overlay.
...
def fractional_day_to_datetime(year, month, day_frac):
    """Convert MPC-style fractional day to timezone-aware datetime."""
...
def parse_mpc(path="I3.txt"):
    """Parse MPC observation file for photometry."""
...
def build_pairs(df, window_days=1):
    """Form color indices between filter pairs within ±window_days."""
...
def plot_colors_and_brightness(pairs, df):
    """Main figure: color evolution + brightness timeline."""
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(9, 6), sharex=True,
        gridspec_kw={"height_ratios": [3, 1.2]}
    )

    # ---------------- COLOR EVOLUTION PANEL ----------------
    ...
    # ---------------- PERIHELION MARKERS ----------------
    peri = PERIHELION
    peri_start = peri - pd.Timedelta(days=5)
    peri_end = peri + pd.Timedelta(days=5)
    for ax in (ax1, ax2):
    ...
    plt.savefig("I3_Color_Trend_v7_6.png", dpi=300)
    plt.close(fig)
    print("✅ Saved: I3_Color_Trend_v7_6.png")
