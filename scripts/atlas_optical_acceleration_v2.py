...
# ------------------------------------------------------------
try:
    # Automatically find the most recent colour-alert CSV
    alert_files = sorted(glob.glob("I3_Color_Alerts_*.csv"))

    if len(alert_files) == 0:
        raise FileNotFoundError("No I3_Color_Alerts_*.csv files found.")

    color_file = alert_files[-1]  # Latest timestamped file
    ...
    # Smooth via 3-day rolling mean
    color_df["color_smooth"] = (
        color_df.groupby("pair")["color"]
        .transform(lambda x: x.rolling(3, center=True, min_periods=1).mean())
    )

    # If needed later in the script:
    # ax.plot(color_df["date"], color_df["color_smooth"], ...)

except Exception as e:
    print(f"⚠️ No colour overlay applied: {e}")
