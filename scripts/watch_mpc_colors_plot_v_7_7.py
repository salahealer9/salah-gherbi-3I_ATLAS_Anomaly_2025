#!/usr/bin/env python3
"""
watch_mpc_colors_plot_v7_7.py
3I/ATLAS (C/2019 Y4) — MPC Color Evolution, Brightness Timeline,
Solar Baselines, Filter Activity, and Pre/Post-Perihelion Δ-Annotations.
...
def plot_colors_and_brightness(pairs, df):
    ...
    # --- PRE/POST-PERIHELION Δ-ANNOTATIONS ---
    text_y = ymax - 0.05 * (ymax - ymin)
    for i, pair in enumerate(pairs["pair"].unique()):
        sub = pairs[pairs["pair"] == pair]
        pre = sub[sub["date"] < peri]["color"].mean()
        post = sub[sub["date"] > peri]["color"].mean()
        ...
        ax1.text(peri_end + pd.Timedelta(days=5),
                 ...
                 f"{pair}: Δ={delta:+.3f} → {trend}",
                 fontsize=8,
                 color="blue" if delta < 0 else "darkred",
                 va="center", ha="left")
    ...
    plt.savefig("I3_Color_Trend_v7_7.png", dpi=300)
    plt.close(fig)
    print("✅ Saved: I3_Color_Trend_v7_7.png")
